...
from core.keyboards.payment_inline import get_choice_course_to_pay_markup, \
    get_payment_builder, \
...
async def choice_course_buts_handler(call: CallbackQuery, bot: Bot) -> None:
    ...
    courses = await UserDAO.get_names_unpurchased_courses(chat_id)
    if len(courses) > 0: 
        await call.message.answer(text='Выберите курс, который хотите преобрести', 
                                reply_markup=get_choice_course_to_pay_markup(courses))
    else: 
        await call.message.answer(text='Вы уже приобрели все курсы или находитель в процессе оплаты')
...
async def confirm_course_selection_handler(call: CallbackQuery, bot: Bot) -> None:
    ...
    await call.message.answer(text=f'Вы выбрали курс {course_name}\n'
                              f'Стоимость {course_cost} рублей\n'
                              f'Подтвердите свой выбор', reply_markup=get_yes_no_payment_builder(course_name).as_markup())
...
async def buying_course_handler(call: CallbackQuery,
                                bot: Bot, 
                                state: FSMContext, 
    ...
    await PurchaseDAO.add(user_chat_id=chat_id,
    ...
    await call.message.answer(text='Оплатите курс. У вас есть 15 минут, после чего тикет закроется\nПосле оплаты обязательно нажмите кнопку "Я оплатил" чтобы получить доступ к курсу', 
    reply_markup=get_payment_builder(url=url, course_name=course_name).as_markup())

    client = Client(settings.P2P_TOKEN)

    apscheduler.add_job(schedule_check_payment, trigger='date', 
                        next_run_time= dt.datetime.now() +  dt.timedelta(minutes=15),    #TODO 
                        kwargs={'chat_id': chat_id, 'pay_token': pay_token,
                                'client': client})
    

async def schedule_check_payment(bot: Bot, chat_id: int, client: Client, pay_token: str):
    history = client.operation_history(label=pay_token)
    if not await check_payment(history=history): 
        await PurchaseDAO.delete_by_pay_token(pay_token=pay_token)
        return await bot.send_message(chat_id=chat_id, text='Вы не успели оплатить счет. Тикет закрыт.\nВы всегда можете попробовать снова')


async def check_payment(history):
    try:
...
async def check_payment_handler(call: CallbackQuery, bot: Bot , apscheduler: AsyncIOScheduler, state: FSMContext) -> None:
    chat_id = call.from_user.id
    state_data: dict = await state.get_data()
    pay_token = state_data['PAY_TOKEN']
    course: Tuple[Course.id, 
                  Course.name, 
                  Course.description, 
                  Course.cost] = state_data['COURSE']
    course_id = course[0]
    ...
    if await check_payment(history=history):
        ...
        await schedule_course_messages(chat_id=chat_id, course_id=course_id, apscheduler=apscheduler)
