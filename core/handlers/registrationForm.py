...
def validate_phone_number(phone_number):
    ...
    # Паттерн для валидации номера
    pattern = re.compile(r'^(\+7|8|7)\d{10}$')

    # Проверяем, соответствует ли номер паттерну
    return bool(pattern.match(phone_number))

async def handler_user_reg_data(msg: Message, state: FSMContext):
    if msg.content_type != 'text': 
        return await msg.answer(f'Данные нужно отправлять в формате текста. Попробуйте снова')
    data = msg.text
    ...
    if state_name== 'GET_FULL_NAME':
        ...
        if not validate_phone_number(data.strip()):
            return await msg.answer(text='Некорректный ввод номера телефона. Попробуйте снова')
        await state.update_data(GET_PHONE=data)
    await msg.answer(f'Вы отправили {bold_data} ✉️\nЧтобы продолжить нажмите'
                     ' "все верно ✅", если нужно повторить ввод, нажмите кнопку "повторить 🔃"', 
                     reply_markup=get_state_manager_but(), parse_mode=ParseMode.HTML)
...
async def handler_accept_but(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if state_name == 'GET_FULL_NAME':
        ...
        await UserDAO.update_by_id(model_id=chat_id, fullname=fullname, phone_number=number, date_registration=registration_date)
...
async def registration_but_handler(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    await call.message.answer(text='❗️Необходимо зарегистрироваться❗️\n✉️ Отправьте ФИО одним смс\n'
