...
async def promo_handler(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    await call.message.answer(
        text="Отправьте промокод, который хотите активировать\nДля отмены вернитесь в меню ",
        reply_markup=get_return_builder().as_markup(),
    )


async def get_promocode_model(promo: str) -> Optional[Promocode]:
...
async def catch_users_promo(
    msg: Message, bot: Bot, state: FSMContext, apscheduler: AsyncIOScheduler
):
    if msg.content_type != "text":
        return await msg.answer(
            f"Промокод нужно отправлять в формате текста. Попробуйте снова"
        )

    promo = msg.text
    promo_model = await get_promocode_model(promo)
    ...
    if not promo_is_active:
        course_model: Course = (
            await CourseDAO.get_all_by_params(id=promo_model.course_id)
        )[0]
        course_name = course_model.name
        unpurchased_courses: list = await UserDAO.get_names_unpurchased_courses(
        ...
        if course_name in unpurchased_courses:
            ...
            await schedule_course_messages(
                chat_id=msg.from_user.id, course_id=course_id, apscheduler=apscheduler
            )
