...
async def first_course_handler(call: CallbackQuery, bot: Bot): #TODO
    ...
    await call.message.answer(text=f'{course_model.description}', reply_markup=get_return_to_start_massage_but())
...
async def check_registration_handler(call : CallbackQuery, bot: Bot):
    ...
    await call.message.answer('Вы не зарегистрированы ❗️', reply_markup=get_but_for_start_registration())
...
async def test_handler(call : CallbackQuery, bot: Bot, apscheduler: AsyncIOScheduler):
    ...
    course: list[Course] = await CourseDAO.get_all()
    courses = [(i.name, i.description) for i in course]
    print(courses)
    
    # modules = await Course_nodulesDAO.get_all_modules(1)
    # for module in modules:
    #     description = module[0]
    #     time = module[1]
    #     # module_order = module[2]
    #     apscheduler.add_job(test_send_mes, trigger='date', 
