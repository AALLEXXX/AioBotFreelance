...
async def question_but_handler(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    await call.message.answer(
        text="Напишите свой вопрос в чат и вам скоро ответят!",
...
async def waiting_message_from_user_handler(msg: Message, bot: Bot, state: FSMContext):
    ...
    if msg.content_type != "text":
        return await msg.answer(
            f"Вопрос нужно отправлять в формате текста. Попробуйте снова"
        )
    ...
    print(admins)
    await UserSupportRequestDAO.add(
        user_id=user_chat_id, question=user_msg, date=datetime.now()
    )
    if len(admins) > 0:
        for chat_id in admins:
            await bot.send_message(
                chat_id=chat_id[0],
                text=f"Вопрос от пользователя @{username}\n-----------------\n"
                f"{user_msg}\n-----------------",
                reply_markup=get_support_keyboard(user_chat_id, msg_id).as_markup(),
            )
        await msg.answer(
            text="Сообщение получено и доставленно менеджерам, ожидайте ответа"
        )
    else:
        await msg.answer(
            text="Админов не найдено. Сообщение не доставленно. Возможно произошел сбой"
        )
